...
export const StudentsManagement = ({ students, courses, applications, sessions = [], readOnly = false, onRefresh }) => {
  ...
  const ligneResultatClasse = (item) => {
    if (item.points_obtenus === null || item.points_obtenus === undefined) return '';
    return Number(item.points_obtenus) >= Number(item.cours_points) / 2
